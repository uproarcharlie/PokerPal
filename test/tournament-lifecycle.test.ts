 ...
 *
 * This test runs through the complete tournament lifecycle:
 * 1. Create a club
 * 2. Create a season
...
async function runTournamentLifecycleTest() {
  logSection("TOURNAMENT LIFECYCLE INTEGRATION TEST");
  ...
  try {
    ...
    for (let i = 0; i < playerNames.length; i++) {
      const [player] = await db.insert(players).values({
        ...
        phone: `021-555-${String(i + 1).padStart(4, "0")}`,
      }).returning();
    ...
    for (const player of createdPlayers) {
      const [registration] = await db.insert(tournamentRegistrations).values({
        ...
        enteringHighHands: Math.random() > 0.5, // 50% enter high hands
      }).returning();
    ...
    logData("Registrations", registrations.map(r => ({
      playerId: createdPlayers.find(p => p.id === r.playerId)?.name
    })));
    ...
    logData("Status", { status: updatedTournament.status });
    ...
    const eliminationOrder = [7, 6, 5, 4, 3, 2, 1, 0]; // Indices in reverse order of finish
    ...
    for (let i = 0; i < eliminationOrder.length; i++) {
      ...
      // Only eliminate players who don't finish in top 3 (they're still in)
    ...
    // Standard payout: 50% / 30% / 20% for top 3
    const payouts = [
    ...
    if (highHandEntrants.length >= 2) {
      const highHandAmount = parseFloat(tournament.highHandAmount as string) / 2; // Split between 2 winners
    ...
    for (let i = 0; i < registrations.length; i++) {
      ...
      logSuccess(`${player.name} awarded ${totalPoints} points (${positionPoints} position + ${pointsSystem.participationPoints} participation)`);
    }
    ...
    const logs = await db.select()
      .from(activityLog)
      .where(eq(activityLog.tournamentId, tournament.id));

    logSuccess("Tournament verified");
    logData("Final Tournament Status", {
      status: finalTournament[0].status,
      prizePoolLocked: finalTournament[0].prizePoolLocked,
      totalPrizePool: finalTournament[0].manualPrizePool,
    });
    ...
    logData("Final Standings", finalRegistrations
      ...
      .map(r => {
        ...
        return {
          ...
          rebuys: r.rebuys,
          addons: r.addons,
          highHand: r.highHandWinner ? `$${r.highHandAmount}` : "-",
        };
      }));

    logData("Activity Log Entries", { count: logs.length });
    ...
    logSection("TEST DATA PRESERVED");
    log("Test data has been kept in the database for inspection", colors.yellow);
    if (createdClub) {
      logData("Club ID", createdClub.id);
      logData("Season ID", createdSeason.id);
      logData("Tournament ID", createdTournament.id);
      log("\nYou can view this tournament in the application!", colors.cyan);
      log("The test data will remain in your database for inspection.", colors.cyan);
    }
